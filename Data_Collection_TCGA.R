...
for (project in projects$Project[1:33]) {
  ...
  exprLogCPM <- readRDS(file=paste0('data/fromTCGA/rData/miRNA_LogCPM_', project, '.RDS'))
  phenoData <- readRDS(file=paste0('data/fromTCGA/rData/Clinical_', project, '.RDS'))
  
  rownames(phenoData) <- phenoData$sample
  
  phenoData <- phenoData[colnames(exprLogCPM),]
  
  expr.tcga.list[[project]] <- exprLogCPM
  
  meta.tcga.list[[project]] <- phenoData
...
colnames(expr.tcga.list[[33]]) == rownames(meta.tcga.list[[33]])


expr.tcga <- do.call(cbind, expr.tcga.list)
dim(expr.tcga)


meta.tcga <- do.call(rbind, meta.tcga.list)

rownames(meta.tcga) <- colnames(expr.tcga)


expr.tcga <- apply(expr.tcga, 2, function(v) round(v,3))

meta.tcga$sample_type <- ifelse(meta.tcga$sample_type=='SolidTissueNormal','Normal','Tumor')


saveRDS(meta.tcga, file='data/fromTCGA/rData/Metadata_TCGA.RDS')
saveRDS(expr.tcga, file='data/fromTCGA/rData/miRNA_Expression_TCGA.RDS')
